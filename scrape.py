...
def validate_page_links(site, page_links, internal_links):
    ...
    for link in page_links:
        ...
        if '/' in link and '#' in link.split('/')[-1]: continue
        if link.startswith('/') and len(link) > 1 and not link.endswith('.pdf') and link not in internal_links and link not in new_found_links:
            new_found_links.append(link)
        elif link.startswith(site) and len(link) > 1 and not link.endswith('.pdf') and link.replace(site, '') not in internal_links and link.replace(site, '') not in new_found_links:
            new_found_links.append(link.replace(site, ''))
        elif link.startswith(site.replace('www.','')) and len(link) > 1 and not link.endswith('.pdf') and link.replace(site.replace('www.',''), '') not in internal_links and link.replace(site.replace('www.',''), '') not in new_found_links:
            new_found_links.append(link.replace(site, ''))
...
def prioritize_page_links(internal_links):
    ...
    internal_links = [link for link in internal_links if sum([text in link.lower() for text in ['/kontakt', 'datenschutz', 'polityka-prywatnosci', 'contact', 'regulamin']])]
...
def scrape(site: str) -> str:
    ...
    site = urlparse('{url.scheme}://{www}{url.netloc}/'.format(url=urlparse(main_page.url), www='www.' if 'www.' in site else '')) #💡
    ...
    for link in internal_links:
        ...
        internal_links.extend(prioritize_page_links(validate_page_links(site.geturl(), page_links, internal_links)))
        ...
        for text in page_text:
            ...
            if sum([clue_word in text.lower() for clue_word in ['zentrale', 'infolinia', 'centrala','headquarters']]):
                # print('⚠️ NEXT could BE DE ZENTRALE ⚠️')
                return_next_num = True
            # for every phone found in text element
            for match in phonenumbers.PhoneNumberMatcher(text, guesse_country_code(site.geturl())):
                ...
                string_phone_parsed = phonenumbers.format_number(match.number, phonenumbers.PhoneNumberFormat.E164)
                # dont accept short numbers (often mistaken with wrong date format: 22.6.2022)
                if len(string_phone_parsed) <= 9: continue
